...
def test_taylor_sin():
    ...
    assert pytest.approx(taylor_sin(x, 10), rel=1e-3) == math.sin(x)  # Check approximation within small error margin
...
def test_taylor_cos():
    ...
    assert pytest.approx(taylor_cos(x, 10), rel=1e-3) == math.cos(x)  # Check approximation within small error margin
...
def test_taylor_tan():
    ...
    assert pytest.approx(taylor_tan(x, 10), rel=1e-3) == math.tan(x)  # Check approximation within small error margin
...
def test_taylor_exp():
    x = 1  # Test e^1
    assert pytest.approx(taylor_exp(x, 10), rel=1e-3) == math.exp(x)  # Check approximation within small error margin
